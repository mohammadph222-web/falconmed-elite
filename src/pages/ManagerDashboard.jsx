...
export default function ManagerDashboard({ user }) {
  const { stats, loading, error } = useDashboardData(user?.id || 'mgr_001')
        ...
        <>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="bg-white rounded-lg shadow p-6">
              ...
              <p className="text-3xl font-bold text-green-600 mt-2">{stats.in_service ?? 0}</p>
            </div>
            <div className="bg-white rounded-lg shadow p-6">
            ...
            <div className="bg-white rounded-lg shadow p-6">
              <p className="text-gray-600 text-sm font-semibold">Avg Wait</p>
              <p className="text-3xl font-bold text-purple-600 mt-2">{Math.round(stats.avg_waiting_time ?? 0)}m</p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <div className="grid grid-cols-2 gap-6">
              <div>
                ...
                <p className="text-2xl font-bold text-green-600 mt-1">{stats.avg_service_time ?? 0} min</p>
              </div>
              <div>
                <p className="text-gray-600 text-sm">Identified Rate</p>
                <p className="text-2xl font-bold text-blue-600 mt-1">
                  {stats.total_patients > 0 ? Math.round(((stats.identified ?? 0) / stats.total_patients) * 100) : 0}%
                </p>
              </div>
            </div>
          </div>
