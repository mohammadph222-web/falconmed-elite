...
import {
  ...
  CheckCircle2, Timer, Bell, Play, CheckCheck, X, Award
} from 'lucide-react'
...
export default function PharmacistDashboard({ user }) {
  ...
  const completedPatients = livePatients.filter((p) => p.status === 'completed')

  // حساب avg service time من المكتملين
  const completedWithTimes = completedPatients.filter(
    (p) => p.called_time && p.finish_time
  )
  const serviceTimes = completedWithTimes.map((p) => {
    const called = new Date(p.called_time).getTime()
    const finished = new Date(p.finish_time).getTime()
    return (finished - called) / 60000
  }).filter((m) => m > 0 && m < 120)
  ...
  return (
    <div className="space-y-6">
      ...
      {stats && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <KpiCard
            ...
            unit="patients"
            color="amber"
            highlight={waitingPatients.length > 5}
          />
          <KpiCard
            icon={<Phone size={22} />}
            label="In Service"
      ...
      {/* Main Grid: Waiting + In Service + Completed */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl">
          ...
          {waitingPatients.length > 0 ? (
            ...
            <EmptyState icon="✨" title="No patients waiting" subtitle="All caught up!" />
          )}
        </div>
        ...
        <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl">
          ...
          {inServicePatients.length > 0 ? (
            ...
            <EmptyState icon="🎯" title="No patients in service" subtitle="Call next patient" />
          )}
        </div>
        ...
        <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl">
          <div className="p-4 border-b border-slate-700/50 flex justify-between items-center">
            <h2 className="text-lg font-bold text-white flex items-center gap-2">
              <CheckCheck size={18} className="text-blue-400" /> Completed
          ...
          {completedPatients.length > 0 ? (
            <div className="divide-y divide-slate-700/30 max-h-[400px] overflow-y-auto">
              {completedPatients.slice(0, 15).map((p) => (
                <div
                  key={p.id}
                  className="p-3 hover:bg-slate-700/30 transition flex justify-between items-center"
                >
            ...
            <EmptyState icon="📋" title="No completed yet" subtitle="Complete a service to see here" />
          )}
        </div>
      </div>
      ...
      {stats && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <PerformanceCard
            ...
            subtitle={`${completedWithTimes.length} completed`}
            color="emerald"
          />
          <PerformanceCard
            label="Quality Score"
